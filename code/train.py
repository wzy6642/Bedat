...
Highlight：
    模型的训练过程分为两个阶段
        1、是对用户相邻两次转发的时间ID差分值进行预测（小型的GPT模型）
        2、是利用时间ID差分值的预测值还原时间ID，并将计算结果加入到行为预测的过程中（较大的GPT模型）
    代码中的transformers包为2.1.1版本、torch包为1.1.0版本
    注意检查config文件中vocab_size参数是否正确
Checkpoints：
    1、我们的时间戳是否可以分别用正弦、余弦函数进行转换以挖掘其周期性与相似性
    2、将时间的先后顺序与Top-k的选择加入到注意力机制中
    3、时间的嵌入矩阵加到哪里比较合适，为什么
    4、<|blank|>是否需要剔除呢？
@author: Zhenyu Wu
"""
#################################
